...
from rpclib.tornado.concurrent import DummyFuture
from rpclib.tornado.log import app_log, gen_log
from rpclib import stackcontext
from rpclib.tornado.util import Configurable
...
try:
    from rpclib.tornado.concurrent import futures
except ImportError:
    futures = None

from rpclib.tornado.platform.auto import set_close_exec, Waker
...
class IOLoop(Configurable):
    ...
    def configurable_default(cls):
        if hasattr(select, "epoll") or sys.platform.startswith('linux'):
            try:
                from rpclib.tornado.platform.epoll import EPollIOLoop
                return EPollIOLoop
            except ImportError:
        ...
        if hasattr(select, "kqueue"):
            ...
            from rpclib.tornado.platform.kqueue import KQueueIOLoop
            return KQueueIOLoop
        from rpclib.tornado.platform.select import SelectIOLoop
        return SelectIOLoop
    ...
    def add_callback_from_signal(self, callback, *args, **kwargs):
        ...
        stackcontext, to avoid picking up the context of the function
    ...
    def add_future(self, future, callback):
        ...
        callback = stackcontext.wrap(callback)
...
class PollIOLoop(IOLoop):
    ...
    def add_handler(self, fd, handler, events):
        self._handlers[fd] = stackcontext.wrap(handler)
    ...
    def add_timeout(self, deadline, callback):
        timeout = _Timeout(deadline, stackcontext.wrap(callback), self)
    ...
    def add_callback(self, callback, *args, **kwargs):
        with self._callback_lock:
            ...
            self._callbacks.append(functools.partial(
                stackcontext.wrap(callback), *args, **kwargs))
...
class _Timeout(object):
    ...
    def __init__(self, deadline, callback, io_loop):
        if isinstance(deadline, (int, long, float)):
            ...
            self.deadline = io_loop.time(
            ) + _Timeout.timedelta_to_seconds(deadline)
        else:
