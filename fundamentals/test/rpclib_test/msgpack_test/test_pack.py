...
from rpclib.msgpack import packb, unpackb, Unpacker, Packer
...
def testIgnoreUnicodeErrors():
    re = unpackb(packb(b'abc\xeddef'), encoding='utf-8',
                 unicode_errors='ignore', use_list=1)
...
def testIgnoreErrorsPack():
    re = unpackb(packb(six.u("abcФФФdef"), encoding='ascii',
...
def testPackFloat():
    assert_equal(
        packb(1.0, use_single_float=True), b'\xca' + struct.pack('>f', 1.0))
    assert_equal(
        packb(1.0, use_single_float=False), b'\xcb' + struct.pack('>d', 1.0))
